...
public class Math_Mult : MonoBehaviour
{
    ...
    private int Skor_Mult, HSkor_Mult;
    public Text SkorKu_Mult, SkorKuF_Mult, HSkorKu_Mult;
    ...
    void Start(){
        Skor_Mult = 0;
        SkorKu_Mult.text = "Skor : " + Skor_Mult;

        textWriter.AddWriter(msgText, "Sudah siap menjawab soal perkalian ?", .1f, true);
    }
    ...
    public void Problem(string operation){
        ...
        if(VarOperation == "multiplication"){
            Operation.text = "X";
        }
        
        //Random Answer 
        tempNumber = Random.Range(2, 150);
        while(tempNumber == finalAns){
            tempNumber = Random.Range(2,150);
        }
        Answer1 = tempNumber;

        tempNumber = Random.Range(2, 100);
        ...
        Debug.Log(firstNumber + " x " + secondNumber + " = " + finalAns);
    ...
    private void Timer(){
        if(timerActive){
            ...
            if(GameAktif && Waktu <= 0){
                ...
                SkorKuF_Mult.text = "Skor : " + Skor_Mult;

                if(Skor_Mult >= PlayerPrefs.GetInt("HSkor_Mult"))
                {
                    PlayerPrefs.SetInt("HSkor_Mult", Skor_Mult);
                }
                HSkorKu_Mult.text = "High Skor : " + PlayerPrefs.GetInt("HSkor_Mult");
